...
function buildRuntimeConversationInstructions(variant: AgentRuntimeVariant): string {
  ...
  if (variant === 'v3') {
    return [
      ...
      '- For common low-risk symptoms (for example flu-like illness, simple headache, common cold), you may give brief immediate home-care support before full triage, but label it as provisional (for example "assuming this is flu-like") and continue with one focused triage question right after.',
      '- Any immediate remedy given before full triage must include one short safety caveat to seek urgent care if danger signs are present.',
      ...
      '- Avoid long generic lists before triage; if giving immediate support first, keep it short, practical, and symptom-specific.',
      ...
      '- Final assessments must be clear and specific, not vague: include likely condition category, why it fits, urgency level, exact red flags, and next-step timeframe in concise language.',
      '- Recommend tests only when they would meaningfully change management or are important for safety/risk clarification; otherwise avoid unnecessary testing lists.',
