...
function Modal({
  ...
  const mobileVerticalPadding = isMobile
    ? `${mobileTopClearance} 12px ${mobileBottomClearance}`
    : "24px";
...
export default function PatientProfilePage() {
  ...
  const inferChannel = (profile: PatientProfile): PatientProfile["channel"] => {
    if (profile.channel === "widget" || profile.channel === "whatsapp")
  ...
  const normalizeDurationSeconds = (
    value: PatientProfile["conversationDurationLastConversationSeconds"],
  ) => {
  ...
  const normalizedProfiles = useMemo(
    () =>
      profiles.map((profile) => ({
        ...
        conversationDurationLastConversationSeconds: normalizeDurationSeconds(
          profile.conversationDurationLastConversationSeconds,
        ),
      })),
    [profiles],
  );
  const selectedProfile = useMemo(
    () =>
      normalizedProfiles.find(
        (profile) => profile.profileId === expandedProfileId,
      ) ?? null,
  ...
  const summary = useMemo(() => {
    const whatsappProfiles = normalizedProfiles.filter(
      (profile) => profile.channel === "whatsapp",
    ).length;
    const widgetProfiles = normalizedProfiles.filter(
      (profile) => profile.channel === "widget",
    ).length;
    const withCustomFields = normalizedProfiles.filter(
  ...
  const deriveConfirmedPatientId = (profile: PatientProfile) => {
    const attributeId =
      profile.attributes?.confirmed_patient_id ||
      profile.attributes?.patient_id;
    const directId = typeof attributeId === "string" ? attributeId.trim() : "";
    if (directId) return directId;
    if (profile.profileId?.trim()) return profile.profileId.trim();
    if (profile.externalUserId?.trim()) return profile.externalUserId.trim();
    return "unconfirmed-patient-id";
  };
    ...
    return normalizedProfiles
      .filter((profile) => {
        ...
        const haystack = [
          ...
          ...Object.entries(profile.attributes ?? {}).flatMap(
            ([key, value]) => [key, value],
          ),
        ]
          .filter(Boolean)
  ...
  const widgetProfiles = useMemo(
    () => filteredProfiles.filter((profile) => profile.channel === "widget"),
    [filteredProfiles],
  );
  const whatsappProfiles = useMemo(
    () => filteredProfiles.filter((profile) => profile.channel === "whatsapp"),
    [filteredProfiles],
  );
  const otherProfiles = useMemo(
    () =>
      filteredProfiles.filter(
        (profile) =>
          profile.channel !== "widget" && profile.channel !== "whatsapp",
      ),
    [filteredProfiles],
  );
  ...
  const syncProfiles = async () => {
    if (!tenantId) return;
    const res = await api<{ profiles: PatientProfile[] }>(
      `/tenants/${tenantId}/xperson-profile?limit=200`,
    );
  ...
  const saveProfile = async (profileId: string) => {
    ...
    try {
      await api(
        `/tenants/${tenantId}/xperson-profile/${encodeURIComponent(profileId)}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            details: {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to save profile changes",
      );
    } finally {
  ...
  const createProfile = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to create patient profile",
      );
    } finally {
      setCreating(false);
    }
  ...
  const deleteProfile = async (profile: PatientProfile) => {
    ...
    const ok = window.confirm(
      `Delete profile ${deriveConfirmedPatientId(profile)}? This action cannot be undone.`,
    );
    if (!ok) return;
    ...
    try {
      await api(
        `/tenants/${tenantId}/xperson-profile/${encodeURIComponent(profile.profileId)}`,
        { method: "DELETE" },
      );
      setProfiles((current) =>
    ...
    return (
      <div style={{ color: "#334155", fontSize: 14, display: "grid", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {isEditing ? (
              <>
                ...
                <button
                  type="button"
                  onClick={() => saveProfile(profile.profileId)}
                  disabled={savingProfileId === profile.profileId}
        ...
        >
          ...
          <div style={metricCardStyle}>
            <span style={{ fontSize: 12, color: "#64748b" }}>External ID</span>
            <strong style={{ overflowWrap: "anywhere" }}>
              {profile.externalUserId || "—"}
            </strong>
          </div>
        ...
        {isEditing ? (
          <div style={{ display: "grid", gap: 12 }}>
            ...
            >
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((v) => ({ ...v, location: e.target.value }))
                  }
                  style={inputStyle}
                />
              </label>
            ...
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <textarea
                value={editForm.attributesText}
                onChange={(e) =>
                  setEditForm((v) => ({ ...v, attributesText: e.target.value }))
                }
                rows={8}
                style={{
                  ...inputStyle,
            ...
            <div style={{ fontSize: 12, color: "#64748b" }}>
              Saving replaces the editable custom field list with exactly what
              is shown here, so removing a line will remove that custom field.
            </div>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            ...
            >
              <div style={metricCardStyle}>
                ...
                <strong>{deriveConfirmedPatientId(profile)}</strong>
              </div>
              <div style={metricCardStyle}>
                <span style={{ fontSize: 12, color: "#64748b" }}>
              ...
              <div style={metricCardStyle}>
                ...
                <strong>{customAttributes.length}</strong>
              </div>
              <div style={metricCardStyle}>
                <span style={{ fontSize: 12, color: "#64748b" }}>Created</span>
                <strong>{formatUpdatedAt(profile.createdAt)}</strong>
              </div>
            </div>
            <div>
              ...
              {customAttributes.length === 0 ? (
                <div style={{ color: "#64748b" }}>
                  No custom patient attributes captured yet.
                </div>
              ) : (
                <div
                  style={{
                ...
                >
                  {customAttributes.map(([key, value], index) => (
                    ...
                    >
                      ...
                      <span
                        style={{ color: "#0f172a", overflowWrap: "anywhere" }}
                      >
                        {value}
                      </span>
                    </div>
        ...
        <div style={{ padding: 18, display: "grid", gap: 14 }}>
          ...
          >
            <div style={{ display: "grid", gap: 6 }}>
              <strong style={{ fontSize: 18, color: "#0f172a" }}>
                {deriveConfirmedPatientId(profile)}
              </strong>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                <span
                  style={{
          ...
          >
            ...
            <div style={metricCardStyle}>
              ...
              <span>{customAttributes.length}</span>
            </div>
          </div>

          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {customAttributes.length > 0 ? (
              customAttributes.slice(0, 4).map(([key, value]) => (
                <span
                  key={key}
                  style={{
                    fontSize: 12,
  ...
  const exportCsv = () => {
    const rows = filteredProfiles.map((profile) => ({
      ...
      confirmedPatientId: deriveConfirmedPatientId(profile),
      phone: profile.phone ?? "",
      location: profile.location ?? "",
      ...
      customFields: Object.entries(profile.attributes ?? {})
  ...
  useEffect(() => {
    if (!tenantId) {
      ...
      setProfiles([]);
      setError(
        "Tenant context is not ready yet. Please refresh or switch tenants.",
      );
      return;
    }
  ...
  return (
    <div style={{ padding: isMobile ? "16px 0" : 0, display: "grid", gap: 16 }}>
      <div>
        <h1 style={{ margin: "0 0 8px 0", fontSize: isMobile ? 24 : 32 }}>
          Patient Profile
        </h1>
        <p style={{ color: "#64748b", marginBottom: 0, maxWidth: 920 }}>
          Profiles captured by Persons (Pipo) from widget and WhatsApp
          conversations. Records display confirmed patient ID values (not
          names), and channels are inferred from IDs when legacy records are
          missing explicit channel metadata.
        </p>
      </div>
      ...
      >
        <div style={metricCardStyle}>
          <span style={{ fontSize: 12, color: "#64748b" }}>Total profiles</span>
          <strong>{summary.total}</strong>
        </div>
        <div style={metricCardStyle}>
          <span style={{ fontSize: 12, color: "#64748b" }}>WhatsApp</span>
          <strong>{summary.whatsappProfiles}</strong>
        </div>
        <div style={metricCardStyle}>
          <span style={{ fontSize: 12, color: "#64748b" }}>Widget</span>
          <strong>{summary.widgetProfiles}</strong>
        </div>
        <div style={metricCardStyle}>
          <span style={{ fontSize: 12, color: "#64748b" }}>
            With custom fields
          </span>
          <strong>{summary.withCustomFields}</strong>
        </div>
      </section>
        ...
        >
          <div>
            <h2 style={{ margin: 0, fontSize: 18, color: "#0f172a" }}>
              Find and review profiles
            </h2>
            <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 13 }}>
              Search identities, filter incomplete records, choose the best
              layout for review, and export the current result set for outreach
              or cleanup work.
            </p>
          </div>
        ...
        >
          <label style={{ display: "grid", gap: 6, minWidth: 0 }}>
            ...
            <input
              ...
              placeholder="Search confirmed patient ID, phone, location, IDs, or custom fields"
              style={inputStyle}
            />
          </label>
        </div>
          ...
          >
            <button
              ...
              style={{
                ...
                color: viewMode === "table" ? "#fff" : "#0f172a",
                borderColor: viewMode === "table" ? "#2563eb" : "#cbd5e1",
              }}
            >
              Table
            </button>
            <button
              ...
              style={{
                ...secondaryButtonStyle,
                flex: isMobile ? 1 : undefined,
                background: viewMode === "card" ? "#2563eb" : "#fff",
                color: viewMode === "card" ? "#fff" : "#0f172a",
                borderColor: viewMode === "card" ? "#2563eb" : "#cbd5e1",
              }}
            >
              Card
            </button>
        ...
        <div style={{ color: "#64748b", display: "grid", gap: 8 }}>
          ...
          {isSlowLoad ? (
            <div style={{ fontSize: 13 }}>
              Still working — if no records exist yet, an empty state will
              appear shortly.
            </div>
          ) : null}
        </div>
      ...
      {!loading && normalizedProfiles.length === 0 ? (
        ...
        >
          No patient profiles yet. Use the Add patient button or enable the
          Patient Profile tool in Agent Settings to capture profiles from
          conversations.
        </div>
      ) : null}
        ...
        >
          ...
          >
            <thead>
              ...
              >
                <th style={{ padding: "12px 14px" }}>Confirmed patient ID</th>
        ...
        >
          <div style={{ display: "grid", gap: 14 }}>
            ...
            >
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((v) => ({ ...v, location: e.target.value }))
                  }
                  style={inputStyle}
                />
              </label>
            ...
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <textarea
                value={createForm.attributesText}
                onChange={(e) =>
                  setCreateForm((v) => ({
                    ...v,
      ...
      {selectedProfile ? (
        <Modal
          title={deriveConfirmedPatientId(selectedProfile)}
